...
export default class MongoObject {
      ...
      if (Array.isArray(val) && val.length > 0) {
        ...
        val.forEach((v, i) => {
          ...
          parseObj(
            self,
            v,
            currentPosition ? `${currentPosition}[${i}]` : String(i),
            `${affectedKey}.${i}`,
            operator,
            adjusted,
            true,
  ...
  getPositionsThatCreateGenericKey(genericKey: string) {
  ...
  private static _keyToPosition = keyToPosition;
